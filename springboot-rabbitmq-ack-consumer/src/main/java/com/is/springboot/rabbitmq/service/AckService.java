...
//money_direct_queue_sli
    ...
    @RabbitHandler
    public void getDlxMessages(String str, Channel channel, Message message) {
...
//            TimeUnit.SECONDS.sleep(2);
    ...
    @RabbitHandler
    public void getDelayMessage(String str, Channel channel, Message message) {
        ...
        try {
            ...
            System.out.println("str:" + str);
            System.out.println("message:" + new String(message.getBody()));
            System.out.println("40秒才收到送消息，日期：" + format.format(date));
//            TimeUnit.SECONDS.sleep(2);
            //告诉服务器收到这条消息 已经被我消费了 可以在队列删掉
            // 这样以后就不会再发了 否则消息服务器以为这条消息没处理掉 后续还会在发
            //第二个参数  表示是否签收多条消息 false 表示不是，true表示是
